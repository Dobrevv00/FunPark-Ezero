...
export default function CtaSection({ content }: { content?: HomePage["cta"] }) {
  ...
  const text = t(
    content?.text,
    "Резервирайте своя престой или събитие днес и си гарантирайте незабравими спомени.",
  );
  // двата бутона днес са с различен текст — не се обединяват
  const ctaMobile = t(content?.ctaLabelMobile, "Виж в TikTok");
  const ctaDesktop = t(content?.ctaLabelDesktop, "Резервирай сега");
